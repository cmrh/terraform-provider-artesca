...
- **S3 client**: 502 / 503 / 504 responses are retried with exponential backoff (4 attempts, 500ms → 8s). 500 is not retried (treated as a real server error). `CreateBucket` has a separate longer retry loop for `InvalidLocationConstraint` to handle location propagation.
...
- Self-hosted CI runner for acceptance and integration test workflows.
