...
type LocationDataSourceModel struct {
	...
	Details           *LocationDetailsDataModel `tfsdk:"details"`
}

// LocationDetailsDataModel mirrors the resource's details model. Sensitive
// fields (secret_key, password) are typed but always empty for a data source
// read because the overlay view masks them.
type LocationDetailsDataModel struct {
